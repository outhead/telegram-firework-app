...
// Запускаем фейерверк после загрузки страницы
$(function(){
	RENDERER.init();
});
...
button.addEventListener('click', () => {
    // Добавляем новый фейерверк при нажатии на кнопку
    RENDERER.fireworks.push(new FIREWORK(RENDERER.width, RENDERER.height, RENDERER));
    RENDERER.fireworks.push(new FIREWORK(RENDERER.width, RENDERER.height, RENDERER));
    RENDERER.fireworks.push(new FIREWORK(RENDERER.width, RENDERER.height, RENDERER));
...
window.addEventListener('resize', () => {
    ...
    RENDERER.width = RENDERER.$container.width();
    RENDERER.height = RENDERER.$container.height();
    RENDERER.contextFireworks.canvas.width = RENDERER.width;
    RENDERER.contextFireworks.canvas.height = RENDERER.height;
    RENDERER.contextTwigs.canvas.width = RENDERER.width;
    RENDERER.contextTwigs.canvas.height = RENDERER.height;
}); 
